  ...
  <div class="content">
    <div class="container-fluid">
      <div class="row mb-4 align-items-center">
        <div class="col-lg-7">
          <h2 class="text-dark mb-1 fw-bold" style="font-size: 1.8rem; letter-spacing: -0.5px;">{{ __('Edit Work Hours Recap') }}</h2>
          <p class="text-muted mb-0" style="font-size: 1.05rem;">{{ __('Update manual recap for employees per location and month.') }}</p>
        </div>
      ...
      <div class="row">
        <div class="col-xxl-8 col-xl-10 mx-auto">
          <div class="card shadow-sm border-0 p-4 p-md-5 rounded-4 border border-light shadow-lg">
            <form action="{{ route('work-recaps.update', $recap) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row g-4">
                    <div class="col-md-4">
                        ...
                        <div class="input-group">
                            ...
                            <select name="location_id" class="form-select bg-dark bg-opacity-50 border-light text-white rounded-end-pill px-4 shadow-none" required>
                                @foreach($locations as $loc)
                                    <option value="{{ $loc->id }}" @selected(old('location_id', $recap->location_id) == $loc->id)>{{ $loc->name ?? $loc->nama ?? __('Location').' '.$loc->id }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="input-group">
                            ...
                            <select name="employee_id" class="form-select bg-dark bg-opacity-50 border-light text-white rounded-end-pill px-4 shadow-none" required>
                                @foreach($employees as $emp)
                                    <option value="{{ $emp->id }}" @selected(old('employee_id', $recap->employee_id) == $emp->id)>{{ $emp->nama ?? $emp->id }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6 mt-5">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" min="0" name="slot_minutes_approved" class="form-control bg-dark bg-opacity-50 border-light text-white rounded-end-pill px-4 shadow-none font-monospace" value="{{ old('slot_minutes_approved', $recap->slot_minutes_approved) }}" required>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6 mt-5">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" min="0" name="attendance_minutes" class="form-control bg-dark bg-opacity-50 border-light text-white rounded-end-pill px-4 shadow-none font-monospace" value="{{ old('attendance_minutes', $recap->attendance_minutes) }}" required>
                        </div>
                    </div>
                    ...
                    <div class="col-12 mt-5 d-flex justify-content-end gap-3">
                        ...
                        <button type="submit" class="btn btn-primary rounded-pill px-5 py-2 fw-bold shadow-lg">
                            <i class="mdi mdi-check-circle-outline me-2"></i>{{ __('Update') }}
                        </button>
                    </div>
                </div>
            </form>
