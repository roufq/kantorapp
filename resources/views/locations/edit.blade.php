                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control @error('default_shift_id') is-invalid @enderror" id="default_shift_id" name="default_shift_id">
                                                ...
                                                @if(isset($assignedShiftsMultiple))
                                                <optgroup label="Assigned to Location — Multiple">
                                                    @foreach($assignedShiftsMultiple as $s)
                                                        <option value="{{ $s->id }}" data-type="multiple" data-scope="assigned" {{ (string)$selectedShiftId === (string)$s->id ? 'selected' : '' }}>
                                                            {{ $s->name }} ({{ $s->getFormattedSchedule() }})
                                                        </option>
                                                    @endforeach
                                                </optgroup>
                                                @endif
                                                @if(isset($assignedShiftsSingle))
                                                <optgroup label="Assigned to Location — Single">
                                                    @foreach($assignedShiftsSingle as $s)
                                                        <option value="{{ $s->id }}" data-type="single" data-scope="assigned" {{ (string)$selectedShiftId === (string)$s->id ? 'selected' : '' }}>
                                                            {{ $s->name }} ({{ $s->getFormattedSchedule() }})
                                                        </option>
                                                    @endforeach
                                                </optgroup>
