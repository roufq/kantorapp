...
<div class="alert alert-warning">
    ...
    @if($task->approval_note)
        <div class="small text-muted mt-1">Alasan penolakan: {{ $task->approval_note }}</div>
    @endif
</div>
@endif
<div class="row">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('tasks.update', $task) }}" method="POST">
                    @csrf
                    @method('PATCH')
                    @if(auth()->user()->hasRole('Super Admin') || auth()->user()->hasRole('Admin Lokasi') || $canSelfEditRejected)
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        @else
                            <div class="form-control-plaintext px-0">
                                {{ auth()->user()->name }} (tidak bisa diubah)
                            </div>
